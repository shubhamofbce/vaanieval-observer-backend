...
The speech handle and the `speech_created` event can, for three of the four
ways LiveKit creates a reply:

| How the reply was created | What it answers | Recorded as |
| --- | --- | --- |
| The automatic answer to a completed user turn — scheduled at once, `user_initiated`, audio input details | The turn that just finished | Merged into that turn, no flag |
...
| A **realtime** model generating server-side while it transcribes — scheduled at once but not `user_initiated` | The speech being transcribed now, whose final transcript may arrive *after* the reply | Its own turn, joined by that transcript, no flag |
| `AgentSession.generate_reply()` called by your own code | Unknowable | Its own turn, `reply_attribution: "inferred"` |

The last row is the only one that stays a guess. It is scheduled and
`user_initiated` exactly like the automatic answer, and differs only in that its
input modality defaults to `"text"` — and your application may call it to answer
the caller or to say something unrelated. The recorder keeps such a reply in a
turn of its own, because merging two exchanges would make a caller's words
appear answered before they were spoken, and then says so. The flag is set on
every span of that turn, including the LLM span, so an interrupted reply that
reported tokens still carries the caveat.
