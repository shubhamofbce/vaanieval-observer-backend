...
| `AgentSession.generate_reply()` called by **LiveKit itself** — committing a realtime turn, retrying a structured output, answering an async tool result, the IVR activity, or a `beta/workflows` prompt | Usually something already under way, but the workflow and IVR helpers prompt *proactively* | Its own turn, joined by the following transcript, `reply_attribution: "inferred"` |
| The stack could not be read — introspection blocked, or the public method's code object unreachable on this build | Unknowable | Its own turn, `reply_attribution: "inferred"` |
...
dozen on 1.7.0 — are not reported as yours. LiveKit's own calls are still
