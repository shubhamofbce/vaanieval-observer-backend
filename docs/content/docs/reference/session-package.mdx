...
| `response.provider_metered_audio_ms` | Exactly what the recogniser reported it will bill for this turn. **What that number measures depends on the provider** — see below |
...
| `response.metered_after_final` | Some of the meter arrived *after* this turn's transcript was final. The usage is still attributed to the turn that earned it rather than to whoever spoke next |
...
To interpret cost per turn, apply your provider's own billing definition. For
how long the caller actually spoke, use `presentation_window` or the turn's
`user_speech_ms` — those are measured from the audio.
...
while that filler is playing, there are two readings and LiveKit does not let
us tell them apart:

1. The partial is an ordinary interim, and the reply that follows is the answer
   to the caller whose filler is playing.
2. The partial is a *preflight* transcript from a second speaker, which starts a
   preemptive generation, and the reply that follows is **their** answer.

Both surface as the same `user_input_transcribed(is_final=False)` event, and
preemptive generation is on by default, so both really happen. The recorder
keeps the reply in a turn of its own, because merging two exchanges would make a
caller's words appear answered before they were spoken. It then sets
`reply_attribution: "inferred"` on that span.
