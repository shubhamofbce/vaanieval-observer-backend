...
| `response.metered_after_final_ms` | How much of the published meter arrived after the caller stopped talking, so you can subtract it. A per-utterance recogniser (OpenAI) puts real speech here; a connection-metering one (Deepgram) puts inter-turn silence here, and nothing in the payload tells them apart |
