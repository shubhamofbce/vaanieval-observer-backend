...
`reply_attribution: "inferred"` on that turn's spans — including the LLM span,
so an interrupted reply that reported tokens still carries the caveat.

The *events* cannot separate the two cases, but the speech handle can: a
preemptive generation stays unscheduled until LiveKit validates its prediction,
while an ordinary reply is marked scheduled immediately. Reading that requires
deferring the binding by one event-loop iteration, which is planned work — so
today this is disclosed rather than resolved.
