...
The speech handle and the `speech_created` event settle three of the six ways
LiveKit creates a reply. The other three emit an event identical in every field
to the automatic answer, so they are settled by what the recorder *observed*
instead — specifically, by which frame asked for the reply:
...
| `AgentSession.generate_reply()` called by **LiveKit itself** — committing a realtime turn, retrying a structured output, answering an async tool result, or the IVR activity; the frame that made the call is inside the `livekit` package | Something already under way | Its own turn, joined by the following transcript, no flag |
...
not hide the call, and LiveKit's own six internal uses of the method are not
reported as yours. The post-tool reply is placed on the turn that ran the tool, which is
