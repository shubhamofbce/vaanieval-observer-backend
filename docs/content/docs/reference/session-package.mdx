...
it. A frame calling itself `generate_reply` on the recorded session is used as a
weaker second reading, for builds where that anchor cannot be resolved at all.
