...
| A **realtime** model generating server-side (`agent_activity.py:1991-2008`) — reports source `generate_reply` like the automatic answer but is not `user_initiated`, and never passes through the emitting function | The speech being transcribed now | Its own turn, joined by that transcript, no flag |
