...
| `response.reply_skipped` | Present when the agent declined to answer this utterance; `"stop_response"` means it raised `StopResponse` |

`provider_metered_audio_ms` is the number the recogniser will bill. It is
