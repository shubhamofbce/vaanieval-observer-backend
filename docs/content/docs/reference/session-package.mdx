...
| `response.provider_metered_audio_ms` | The recogniser's **connection** meter, not this turn's speech |
...
| `response.metered_after_final` | The provider metered this turn *after* its transcript was final. The usage is still attributed to the turn that earned it rather than to whoever spoke next |
| `response.metered_after_final_ms` | How much of the published meter arrived after the caller stopped talking, so you can subtract it |
| `response.metering_scope` | What the meter is a measurement *of*: `utterance` (all of it was reported while the caller was still speaking, so it is comparable to the recorded audio), `connection` (all of it arrived afterwards, so none of it is a measurement of this turn's speech), or `mixed`. Deepgram flushes every five seconds off frames streamed, silence included, so it commonly reports `connection`; OpenAI meters per utterance. The amount itself is never adjusted — it is still what the provider will invoice |
...
`provider_metered_audio_ms` is the number the recogniser will bill, summed over
every increment it reported while this turn was open. It is
reported on a fixed timer covering everything streamed to the socket, including
silence between turns, so a tick lands on whichever turn happens to be open and
can exceed the span that carries it. For how long the caller actually spoke, use
`presentation_window` or the turn's `user_speech_ms`.
