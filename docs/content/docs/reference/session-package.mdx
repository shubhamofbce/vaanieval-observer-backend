...
| `AgentSession.generate_reply()` called by **LiveKit itself** — committing a realtime turn, answering an async tool result, the IVR activity, or a `beta/workflows` prompt | Usually something already under way, but the workflow and IVR helpers prompt *proactively* | Its own turn, joined by the following transcript, `reply_attribution: "inferred"` |
| LiveKit **reissuing** a reply for a run that already exists — `RunResult._maybe_retry_output()` (`run_result.py:292`) after a structured output fails to validate, or `realtime_fallback_adapter.py:394`/`:445` when a realtime session drops to a text model | Whoever started that run; the event carries no run identity to say who | Its own turn, **not** joined by the following transcript, `reply_attribution: "inferred"` |
...
the method entirely is recognised too, because the reply is anchored on the
function that *emits* the event (`AgentActivity._generate_reply()`) and, failing
that, on a frame calling itself `generate_reply` on an `AgentSession`. LiveKit's own calls are still
